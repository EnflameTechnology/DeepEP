...
run_installer() {
    ...
    if [ "$DEBUG" -eq 1 ]; then
        ...
        local log_file
        log_file="$(mktemp /tmp/fa_driver_XXXXXX.log)"
...
# TODO: update image, caps & aten need to be updated
DOCKER_IMAGE="registry-egc.enflame-tech.com/artifacts/deepep:torch2.11.0-TR3.8.106-ubuntu2204"
CONTAINER_NAME="tmp_container"
TOTAL_STEPS=4
STEP=0

if [ -z "$DOCKER_IMAGE" ]; then
    echo -e "${RED}[ERROR]${NC} DEEPEP_DRIVER_IMAGE is not set."
    echo "Set DEEPEP_DRIVER_IMAGE to a container image that provides /driver before running this script."
    exit 1
fi
...
mkdir -p tmp
log_info "Copying /driver from container to ./tmp/ ..."
docker cp "$CONTAINER_NAME:/driver" tmp/
log_ok "Driver files copied to $REPO_ROOT/tmp/driver"

next_step "Install driver (requires sudo)"
DRIVER_DIR="$REPO_ROOT/tmp/driver"
cd "$DRIVER_DIR"
