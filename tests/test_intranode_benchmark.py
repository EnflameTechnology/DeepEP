...
def test_main(args: argparse.Namespace, num_sms: int, local_rank: int, num_ranks: int, rank: int,
    ...
    for current_x in filter(lambda elem: elem is not None, (x_e4m3, x)):
        ...
        # Tune combine for current data type
        ...
        for nvl_chunk_size in tuple(range(1, 2, 1)) + (0, ):
            ...
            if use_kineto:
                # Benchmark combine using bench_kineto()
                # Use 'intranode::combine<' for precise matching
                dist.barrier()
                kineto_stats = bench_kineto(lambda: buffer.combine(**tune_args), kernel_names='intranode::combine<',
                                            barrier_comm_profiling=True, suppress_kineto_output=True, return_stats=True)
                combine_avg_t = kineto_stats['avg']
                combine_min_t = kineto_stats['min']
                combine_max_t = kineto_stats['max']
            else:
                # Benchmark combine using bench()
                combine_avg_t, combine_min_t, combine_max_t = bench(lambda: buffer.combine(**tune_args))
            
            if combine_avg_t < best_combine_time and nvl_chunk_size > 0:
        ...
        print(f'[rank {rank}] ({data_type}) Dispatch{method_tag} bandwidth: {dispatch_bandwidth:.2f} GB/s, avg_t={dispatch_latency_us:.2f} us, '
              ...
              f'Combine{method_tag} bandwidth: {combine_bandwidth:.2f} GB/s, avg_t={combine_latency_us:.2f} us, '
        ...
        if rank == 0:
            print(f'[avg across {num_ranks} ranks] ({data_type}) Dispatch{method_tag} bandwidth: {dispatch_bandwidth_mean:.2f} GB/s, avg_t={dispatch_latency_us_mean:.2f} us | '
                  f'Combine{method_tag} bandwidth: {combine_bandwidth_mean:.2f} GB/s, avg_t={combine_latency_us_mean:.2f} us', flush=True)
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    log_file = f"{rank}_test_intranode_benchmark_{log_timestamp}.log"
