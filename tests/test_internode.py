...
import deep_ep
from utils import init_dist, bench, bench_kineto, calc_diff, create_grouped_scores, inplace_unique, per_token_cast_to_fp8, per_token_cast_back, hash_tensor, build_base_parser, add_sms_arg
...
def test_main(args: argparse.Namespace, num_sms: int,
    ...
    for previous_mode in (False, ):
        for async_mode in (False, ):
            ...
            for current_x in ((x, x_pure_rand, x, x_pure_rand)*50):
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    log_file = f"{rank}_test_internode_{log_timestamp}.log"
...
if __name__ == '__main__':
    parser = build_base_parser('Test internode EP kernels',
                               defaults={'num_tokens': 10240, 'num_experts': 256},
