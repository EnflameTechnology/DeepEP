...
def run_dispatch_test(x_list: List[torch.Tensor], topk_idx: torch.Tensor, num_tokens: int, num_experts: int,
                      ...
                      round_scale: bool = False, use_ue8m0: bool = False, do_check: bool = True) -> int:
...
def test_main(num_tokens: int, hidden: int, num_experts: int, num_topk: int,
    ...
    configurations = [
        ("BF16 (default, return_recv_hook=False)", False, False, False, False),
        ("BF16 (return_recv_hook=True)", True, False, False, False),
    ]
    
    # If FP8 test is enabled, add FP8 configurations
    if test_fp8:
        configurations.extend([
            ("FP8 (return_recv_hook=False)", False, True, False, False),
            ("FP8 (return_recv_hook=True)", True, True, False, False),
            ("FP8 with round_scale (return_recv_hook=False)", False, True, True, False),
            ("FP8 with round_scale (return_recv_hook=True)", True, True, True, False),
            ("FP8 with round_scale and use_ue8m0 (return_recv_hook=False)", False, True, True, True),
            ("FP8 with round_scale and use_ue8m0 (return_recv_hook=True)", True, True, True, True)
        ])
    
    # Run tests for all configurations
    results = {}
    combined_hash = 0  # Initialize combined hash value for XOR operation
    for config_name, return_recv_hook, dispatch_use_fp8, round_scale, use_ue8m0 in configurations:
        ...
        hash_value = run_dispatch_test(
            x_list, topk_idx, num_tokens, num_experts, rank, num_ranks, group, buffer,
            return_recv_hook=return_recv_hook, dispatch_use_fp8=dispatch_use_fp8,
            round_scale=round_scale, use_ue8m0=use_ue8m0
        )
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    log_file = f"{rank}_test_low_latency_split.log"
