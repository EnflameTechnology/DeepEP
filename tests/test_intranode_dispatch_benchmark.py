...
#os.environ["TOPS_VISIBLE_DEVICES"]="1,8,9,10,11,12,13,14"
#os.environ["TOPS_VISIBLE_DEVICES"]="0,1,2,3,4,5,6,7"
#os.environ["TOPS_VISIBLE_DEVICES"]="4,5,12,13"
...
def test_main(args: argparse.Namespace, num_sms: int, local_rank: int, num_ranks: int, rank: int,
    ...
    for current_x in filter(lambda elem: elem is not None, (x_e4m3, x)):
        ...
        for nvl_chunk_size in [0, 2, 4, 8]:
            ...
            #tune_args = {'x': current_x, 'num_tokens_per_rank': num_tokens_per_rank,  'is_token_in_rank': is_token_in_rank,
            #                         'num_tokens_per_expert': num_tokens_per_expert, 'config': config, 'async_finish': async_mode}
            ...
            if use_kineto:
                ...
                kineto_stats = bench_kineto(lambda: buffer.dispatch(**tune_args), kernel_names='intranode::dispatch<',
                                            barrier_comm_profiling=True, suppress_kineto_output=True, return_stats=True, num_tests=500)
            ...
            print(f"nvl_chunk_size {nvl_chunk_size}, dispatch_avg_t {dispatch_avg_t}, dispatch_min_t {dispatch_min_t} dispatch_max_t {dispatch_max_t}")
        ...
        print(f'[rank {rank}] ({data_type}) Dispatch{method_tag} bandwidth: {dispatch_bandwidth:.2f} GB/s, avg_t={dispatch_latency_us:.2f} us, '
              f'min_t={dispatch_min_us:.2f} us, max_t={dispatch_max_us:.2f} us best_dispatch_time={best_dispatch_time}', flush=True)
        ...
        if rank == 0:
            ...
            print(f'[tuning] ({data_type}) Best dispatch: SMs {best_dispatch_config[0]}, NVL chunk {best_dispatch_config[1]}, '
                  f'nvl_recv_bytes {nvl_recv_bytes / 1e6} MB'
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    log_file = f"{rank}_test_intranode_dispatch_benchmark_{log_timestamp}.log"
...
if __name__ == '__main__':
    parser = build_base_parser('Test intranode EP kernels',
                               defaults={'num_tokens': 4096 * 4, 'hidden': 4096, 'num_experts': 192})
