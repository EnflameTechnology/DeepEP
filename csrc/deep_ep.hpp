...
struct Buffer {
    EP_STATIC_ASSERT(NUM_MAX_NVL_PEERS <= 256, "The number of maximum NVLink peers must not exceed 256");
...
private:
    ...
    int* cached_value_ptrs = nullptr;

    // NVSHMEM Buffer
    int64_t num_rdma_bytes;
    void* rdma_buffer_ptr = nullptr;
    void* rdma_peer_base_addr_dev = nullptr;
    void* rdma_ibgda_info_addr_dev = nullptr;
    void* qp_shared_states_addr_dev = nullptr;
    uint64_t rdma_peer_base_addr[NUM_MAX_NVL_PEERS] = {0};
    ...
    int local_ranks;
    uint64_t buffer_ptrs_edf[NUM_MAX_NVL_PEERS];
    uint64_t barrier_signal_ptrs_edf[NUM_MAX_NVL_PEERS];
    topsIpcMemHandle_t ipc_handles[NUM_MAX_NVL_PEERS];

    // Stream for communication
    torch_gcu::GCUStream comm_stream;
    ...
    epComm_t epComm = nullptr;

    // Slave mode registered memory handles (for cleanup via topsRoceUnregMem)
    std::vector<void*> slave_reg_addrs_;

public:
    const int NUM_NVL_RANKS_INTERNODE = 8;
    bool is_internode = false;
    primitives* prims = nullptr;
    Buffer(int rank, int num_ranks, int64_t num_nvl_bytes,
        int64_t num_rdma_bytes, bool low_latency_mode,
        bool explicitly_destroy, bool is_internode,
        const std::vector<uint8_t>& root_unique_id);
    ...
    // Getter methods for rank and num_ranks (used by test kernels)
    int get_rank() const { return rank; }
    int get_num_ranks() const { return num_ranks; }

    pybind11::bytearray get_local_ipc_handle() const;

    uint64_t get_local_edf() const;

    pybind11::bytearray get_unique_id() const;

    pybind11::bytearray get_local_nvshmem_unique_id() const;

    torch::Tensor get_local_buffer_tensor(
    ...
    // Returns include optional `actual_num_recv_tokens` (shape [1], int32)
    // after `send_head`:
    // set when num_worst_tokens > 0; written by notify_dispatch on device
    // (no CPU sync / D2D copy).
    std::tuple<
        torch::Tensor, std::optional<torch::Tensor>,
        std::optional<torch::Tensor>, std::optional<torch::Tensor>,
        std::vector<int>, torch::Tensor, torch::Tensor, torch::Tensor,
        torch::Tensor, torch::Tensor, std::optional<torch::Tensor>,
    ...
    intranode_combine(const torch::Tensor& x,
        ...
        const torch::Tensor& send_head, int num_experts,
        const Config& config,
    ...
    std::tuple<
        ...
        std::optional<EventHandle>>
    internode_notify_dispatch(const torch::Tensor& num_tokens_per_rank,
        const torch::Tensor& num_tokens_per_rdma_rank,
        const torch::Tensor& num_tokens_per_expert,
        const torch::Tensor& is_token_in_rank,
        int num_channels, int hidden_int4, int num_scales,
        int num_topk, int expert_alignment,
        const Config& config, std::optional<EventHandle>& previous_event,
        bool async);
    ...
    // Test / debug helper:
    //      read host-mapped MoE counters after notify_dispatch.
    // Returns (moe_recv_counter, moe_recv_rdma_counter,
    // moe_recv_expert_counter[0:num_local_experts]).
    std::tuple<int, int,
        std::vector<int>> get_moe_recv_info(int num_local_experts) const;
    ...
    low_latency_dispatch(const torch::Tensor& x, const torch::Tensor& topk_idx,
        ...
        const std::optional<torch::Tensor>& fp8_quant_scale = std::nullopt);
