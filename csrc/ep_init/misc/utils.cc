...
epResult_t getBusId(int topsDev, int64_t *busId) {
  ...
  TOPS_CHECK(topsDeviceGetPCIBusId(busIdStr, sizeof(busIdStr), topsDev));
 ...
 *
 * This string can be overridden by using the EP_HOSTID env var.
 */
...
uint64_t getHostHash(void) {
  ...
  const char* hostId = epGetEnv("EP_HOSTID");
  if (hostId && strlen(hostId) > 0) {
    INFO(EP_ENV, ENV_FORMAT_STR, "EP_HOSTID", hostId);
  ...
  TRACE(EP_INIT,"unique hostname '%s'", hostHash);
...
uint64_t getPidHash(void) {
  ...
  pname[plen+len]='\0';
  TRACE(EP_INIT,"unique PID '%s'", pname);
...
// reverse elements in an array from start to end
...
// left rotate array by 'shift' positions
void leftRotate(int arr[], int size, int shift) {
    if (size <=0) return;

    shift = shift % size; // Adjust shift value if it exceeds array length
